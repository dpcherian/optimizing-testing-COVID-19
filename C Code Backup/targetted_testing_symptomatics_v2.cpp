...
bool quarantine_confined = true;               // Change to false if you don't want to confine individuals
...
void Targetted_Run(double dt, double rate[][n_states], double Tpars[][4], int tf, bool lock_homes, double begin_at, double test_frac){
  ...
  std::fill_n(loc_confined_time,n_pop,-1000);     // Set all values of loc_confined_time to -1000
  ...
  double alpha = 1 - Cpars[5];                            // Quantity by which V is reduced per hospitalised individual
  ...
  bool start_testing = false;   // Variable to decide whether testing should start
  ...
  while(t<tf){
    ...
    for(int i=0; i< n_loc; i++){
      ...
      double V = N - alpha*n_per_location[i][H];   // Spatial damping parameter (adjusted by alpha for hospitals)
      ...
      r[0] = rate[S][A] * n_per_location[i][S] * 1/V *
                                                (Cpars[0]*(n_per_location[i][A]- conf_by_state_in_loc[1]*alpha) +   // SA
                                                 Cpars[1]*(n_per_location[i][P]- conf_by_state_in_loc[2]*alpha) +   // SP
                                                 Cpars[2]*(n_per_location[i][MI]-conf_by_state_in_loc[3]*alpha) +   // S-MI
                                                 Cpars[3]*(n_per_location[i][SI]-conf_by_state_in_loc[4]*alpha) +   // S-SI
    ...
    if(t>=day){
      ...
      if(start_testing){
        ...
        for(int i=0; i<n_pop;i++){
          if((pop[i][0]==MI||pop[i][0]==SI) && day>=next_test_date[i]){
            list_of_sym[n_sym] = i; n_sym++;
          }
        }

        if(n_sym>0){
          int targetted_tests_done_this_dt = std::min(tests_remaining_today,n_sym);  // Tests done this dt is the minimum of the tests available and
          ...
          for(int j=0; j<targetted_tests_done_this_dt;j++){
            ...
            if(day >= next_test_date[si] && being_tested[si]==false && tests_remaining_today>0){
              ...
              if(quarantine_when_sample_taken==true){is_confined[si] = quarantine_confined;}      // Quarantine as soon as sample is taken
      ...
      for(int i=0; i<n_pop; i++){
        if(start_testing==true){
          ...
          double p = test_frac/100;     // Probability of a person being tested today randomly
          ...
          if(day>= next_test_date[i] && being_tested[i] == false){
            if(uniform()<p && tests_remaining_today>0){
              ...
              if(quarantine_when_sample_taken==true){is_confined[i] = quarantine_confined;}         // QUarantine as soon as sample is taken
    ...
    for(int i=0;i<n_pop;i++){
      ...
      if(day >= result_declared_date[i] && being_tested[i]==true){
        if(start_testing==false){printf("ERRROR!!!!!!!!!\n");}
        ...
        if(test_result[i]==1 && pop[i][0] != H){  // If the result is positive, and the person hasn't already moved to a hospital
          ...
          next_test_date[i] += 13; // TO make compatible with applet.
        }
      ...
      if(is_confined[i] == false && loc_confined[pop[i][3]]==false){
        ...
        if(locs_changed==1){
          ...
          // If they're at home (and isn't hospitalised), move them to work and vice versa
...
int main() {
  ...
  double Tpars[2][4] = {{0.75, 0.98, 1, 0},
                        {1.0,  1.0,  1, 1}};
