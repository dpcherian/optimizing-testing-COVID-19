...
bool quarantine_confined = false;               // Change to false if you don't want to confine individuals
  ...
  while(t<tf){
    ...
    for(int i=0; i< n_loc; i++){
      ...
      if(quarantine_confined == true){
        printf("Before: Loc %d \n",i);
        for(int ii=0;ii<n_states;ii++){printf("%d",conf_by_state_in_loc[ii]);}
    ...
    for(int i=0;i<n_pop;i++){
      ...
      if(day >= result_declared_date[i] && being_tested[i]==true){
        ...
        if(test_result[i]==1 && pop[i][0] != H){  // If the result is positive, and the person hasn't already moved to a hospital
          ...
          is_confined[i] = quarantine_confined; // Confine them
