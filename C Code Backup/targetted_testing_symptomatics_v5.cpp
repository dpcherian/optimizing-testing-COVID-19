...
void writetofile(int output[][op_width], int tf, double Tpars[2][4], double begin_at, double test_frac, double time_taken, int details[9]){
  ...
  sprintf(str,"Targetted-Testing_BeginAt_%g_DTR_%g_RAT_%g_%g_PCR_%g_%g_%s.txt",begin_at,test_frac,Tpars[0][0],Tpars[0][3],Tpars[1][0],Tpars[1][3],dateString);
...
void Targetted_Run(double dt, double rate[][n_states], double Tpars[][4], int tf, bool lock_homes, bool quarantine_when_sample_taken, double begin_at, double test_frac){
  ...
  while(t<tf){
    ...
    for(int i=0; i< n_loc; i++){
      ...
      for(int j=0; j<n_states; j++){ N += n_per_location[i][j];} // Find total number in  location
     ...
     r[1] = rate[S][P] * n_per_location[i][S] * 1/V *
      ...
      // // Checking for negative populations
      // for( int r1=0;r1<n_events;r1++){
      //   if(r[r1]<0){printf("Negative rate! DAMN! %f\n",r[r1]);}
      // }

      while(flag==false){

        for(int j=0; j<n_events; j++){K[j] = poisson(r[j]*dt);} // Get number of people per event (poisson distributed)
        ...
        for(int j=0; j<n_states;j++){
          for(int k=0; k<n_states;k++){n_removed[j] += transitions[j][k]; }
        }
      ...
      for(int j=0; j<n_states; j++){
       ...
       for(int j=0; j<n_states;j++){ backup[j] = n_per_location[i][j];}
            ...
            for(int k=0;k<n_states;k++){
                ...
                for(int m=0; m<N;m++){
                  ...
                  if(pop[ind[m]][0]==j && done[m]==false){
                    ...
                    if(k==R && j==H){ // If they recover from Hospital (CHANGED)
    ...
    if(t>=day+1){
        ...
        for(int i=0;i<n_pop;i++){
            ...
            if(day == result_declared_date[i] && being_tested[i]==true){
              ...
              if(test_result[i]==1 && pop[i][0] != H){  // If the result is positive, and the person hasn't already moved to a hospital
                ...
                next_test_date[i] += 14; // TO make compatible with applet. (CHECK!)
              }
      ...
      // Moving people around deterministically
      ...
      // int locked = 0;
      // for(int hq=0;hq<n_loc;hq++){if(loc_confined_time[hq]>=0){locked++;}}

      // printf("%i Confined: %i  Confined homes: %i Unlocked today: %i\n", day, sum(is_confined,n_pop), sum(loc_confined,n_loc) ,unlockedtoday);
    }
...
int main() {
  ...
  bool lock_homes = true;                       // Change to false if you don't want homes to be locked down
