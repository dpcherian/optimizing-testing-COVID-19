...
void writetofile(int output[][op_width], int tf, double Tpars[2][4], double begin_at, double test_frac, double time_taken, int details[9],int iter){
  ...
  strftime(dateString,sizeof(dateString),"%s",timeinfo);
...
void Targetted_Run(double dt, double rate[][n_states], double Tpars[][4], int tf, bool lock_homes, bool quarantine_when_sample_taken, double begin_at, double test_frac,int iter){
  ...
  while(t<tf){
    ...
    if(midday_move_completed == false && t - day >  0.5){
      // printf("%lf %i\n",t, day );         //TEST: to remove
      // printf("Moved at time t=%lf\n",t); //TEST: to remove
    ...
    for(int i=0; i< n_loc; i++){
     ...
     r[1] = rate[S][P] * n_per_location[i][S] * 1/V *
      ...
      // if(r[0]>0 || r[1]>0){
      // printf("S %i A %i P %i MI %i SI %i H %i R %i V %lf\n", n_per_location[i][S],n_per_location[i][A],n_per_location[i][P],n_per_location[i][MI],n_per_location[i][SI],n_per_location[i][H],n_per_location[i][R],V);
      // printf("r[0] %lf\n",r[0]);
      // printf("r[1] %lf\n",r[1]);} //TEST: to remove

      bool flag = false;
      counter = 0;  // counter to check number of runs so that poisson means don't stay negative too long.

      // TEST: To remove. Checking for negative populations
      const char *events[n_events]={"SA","SP","AR","PMI","PSI","MIR","SIR","SIH","HR"};
      for( int r1=0;r1<n_events;r1++){
        if(r[r1]<0){printf("Negative rate for event %s %f\n",events[r1], r[r1]);}
      }
      ...
      while(flag==false){
        ...
        for(int j=0; j<n_events; j++){K[j] = poisson((double)(r[j]*dt));} // Get number of people per event (poisson distributed)
        // for(int j=0; j<n_events; j++){K[j] = poisson((double)r[j]*dt); printf("r[%i]*%lf = %lf\n",j,dt,r[j]*dt );} // Get number of people per event (poisson distributed) // TEST: To remove

        // if(K[0]>0 || K[1]>0 || K[2]>0 || K[3]>0 || K[4]>0 || K[5]>0 || K[6]>0 || K[7]>0 || K[8]>0){printf("Location %i\n",i);  for(int k=0;k<n_events;k++){printf("%i ",K[k]);}printf("\n"); } // TEST: to remove
        if(K[0]==0 && K[1]==0 && K[2]==0 && K[3]==0 && K[4]==0 && K[5]==0 && K[6]==0 && K[7]==0 && K[8]==0){flag=true; continue;} // If no transitions occur, skip.
        ...
        // printf("Location %i\n",i);                                            // TEST: To remove
        // for(int j=0; j<n_states;j++){printf("%i ", backup[j]);}printf("\n");  // TEST: To remove
        ...
        for(int j=0; j<n_states;j++){
          ...
          // printf("%i ", n_per_location[i][j]); // TEST: To remove
        }
        // printf("\n"); // TEST: To remove
        // printf("*******\n"); // TEST: To remove
        ...
        if(n_per_location[i][S] <0 || n_per_location[i][A]<0 || n_per_location[i][P]<0 || n_per_location[i][MI]<0 ||
           n_per_location[i][SI]<0 || n_per_location[i][R]<0 || n_per_location[i][H]<0){ // If any n_per_location<0
             // printf("Rolling back\n");
             dt = dt/2;
             for(int j=0; j<n_states;j++){n_per_location[i][j]=backup[j];} // Roll back to backup

             counter++; // Counter to keep track of number of retries
             if(counter == 9){printf("%s\n", "Fatal error: 10 retries didn't get rid of negative populations!");exit(1);}

           }
         else{flag=true;dt=old_dt;}

      } // END WHILE FLAG Loop

      /************* CHANGE STATE OF POP **************/

      if(K[0]==0 && K[1]==0 && K[2]==0 && K[3]==0 && K[4]==0 && K[5]==0 && K[6]==0 && K[7]==0 && K[8]==0){continue;} // If no transitions occur, skip this too.

      int sum_t = 0;                                                                       // TEST: To remove
      for(int j=0;j<n_states;j++)for(int k=0;k<n_states;k++){sum_t += transitions[j][k];}  // TEST: To remove

      // printf("Location %i\n",i);
      // for(int j=0;j<N;j++){printf("%i ", ind[j]);}printf("\n");   //TEST: to remove

      shuffle(0,N,ind); // Shuffle list of people currently in locations

      // for(int j=0;j<N;j++){printf("%i ", ind[j]);}printf("\n");  //TEST: to remove

      int total_transitions=0; // TEST: to remove

      for(int j=0;j<N;j++){                    // Loop over the people currently in the location

        for(int from=0;from<n_states;from++){  // Loop over possible "from" states
          if(pop[ind[j]][0]==from){            // If the person's current state happens to be "from"
            for(int to=0;to<n_states;to++){    // Loop over possible "to" states
              if(transitions[from][to]>0){     // If such a transition must occur,
                pop[ind[j]][0] = to;           // Send this person to the "to" state.
                n[from]--; n[to]++;            // Change the values of n[from] and n[to]
                transitions[from][to]--;       // Reduce the number of "from -> to" transitions
                total_transitions++;
                ...
                if(to==H){
                  // Move them to the hospital
                    ...
                    n_per_location[ pop[ind[j]][3] ][H]--; // Decrement number of k (=H) in current location
                    pop[ind[j]][3] = h;                    // Send them to a random hospitals
                    n_per_location[ pop[ind[j]][3] ][H]++; // Increment number of k (=H) in current location
                }
                else if(from==H && to == R){
...
//                  is_confined[ind[j]] = false; // Remove confinement
                ...
                break;
              } // End if condition (transitions>0)
      ...
      if(total_transitions!=sum_t){printf("ERROR! Old transitions %i New transitions %i\n",sum_t,total_transitions );}
