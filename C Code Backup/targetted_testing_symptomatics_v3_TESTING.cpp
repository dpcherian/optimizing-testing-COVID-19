...
void Targetted_Run(double dt, double rate[][n_states], double Tpars[][4], int tf, double begin_at, double test_frac){
  ...
  while(t<tf){
    ...
    for(int i=0;i<n_pop;i++){
      ...
      if(is_confined[i] == false){// && loc_confined[pop[i][3]]==false){
        ...
        // else if (loc_confined[pop[i][3]]==true){
        //   printf("Location locked? How?");
        // }
      }
    }
  ...
  int details[] = {false, quarantine_confined, lq_tests_conducted, hq_tests_conducted, tests_conducted, results_declared, locations_moved, hcw_recovered,hcw};
