...
void writetofile(int output[][op_width], int tf, double Tpars[2][4], double begin_at, double test_frac, double time_taken, int details[9],int iter){
  ...
  sprintf(str,"../tmp/Targeted-Testing_BeginAt_%g_DTR_%g_RAT_%g_%g_PCR_%g_%g_%lf%lf-%i.txt",begin_at,test_frac,Tpars[0][0],Tpars[0][3],Tpars[1][0],Tpars[1][3],uniform(),uniform(),iter);
  ...
  while(t<tf){
    ...
    for(int i=0; i< n_loc; i++){
      ...
      for(int j=0;j<N;j++){                    // Loop over the people currently in the location
        ...
        bool done = false; // Variable to keep track of when this person is done, to break out of ("from") loops.
       ...
       r[S][P] = rate[S][P]   * 1/V * (Cpars[0]*(n_per_location[i][A]- conf_by_state_in_loc[A]*alphaQ) +    // SA
        ...
        for(int j=0;j<n_states;j++){
          for(int k=0;k<n_states;k++){exit_rate[j] += r[j][k];}
        }


        for(int from=0;from<n_states;from++){  // Loop over possible "from" states
          if(pop[ind[j]][0]==from && uniform()<exit_rate[from]*dt){            // If the person's current state happens to be "from", and the person is selected to move
            double p = uniform();
            double temp=0;
