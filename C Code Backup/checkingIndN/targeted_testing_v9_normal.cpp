...
const int n_asym = mult*10;      // Initial number of asymptomatics

const int maxppl=n_pop;
...
void createPopulation(){
  ...
  for (int i=0; i<n_pop;i++){
    ...
    // std::fill_n(people_linked_to,n_pop,-1);  // Set all values of last_test_type to -1
    memset(people_linked_to, -1, sizeof(people_linked_to[0][0]) * maxppl * n_loc);

    // temp=0;
    // for(int k=0;k<n_loc;k++){
    //   for(int l=0;l<n_pop;l++)
    //   if(people_linked_to[k][l]==-1)break;
    //   else temp++;
    // }
    // printf("After %i\n",temp);
...
void writetofile(int output[][op_width], int tf, double Tpars[2][4], double begin_at, double test_frac, double time_taken, int details[9],int iter){
  ...
  sprintf(str,"./Targeted-Testing_dt_%g_BeginAt_%g_DTR_%g_RAT_%g_%g_PCR_%g_%g_%lf%lf-%i.txt",old_dt,begin_at,test_frac,Tpars[0][0],Tpars[0][3],Tpars[1][0],Tpars[1][3],uniform(),uniform(),iter);
...
void Targetted_Run(double dt, double Tpars[][4], int tf, bool lock_homes, bool quarantine_when_sample_taken, double begin_at, double test_frac,int iter){
  ...
  int tests_available_daily = (int)(test_frac/100 * n_pop);
  int tests_remaining_today = (int)(test_frac/100 * n_pop);

  int lq_tests_daily = (int)(Tpars[0][3]*tests_available_daily);
  int lq_tests_today = (int)(Tpars[0][3]*tests_available_daily);

  int hq_tests_daily = (int)(Tpars[1][3]*tests_available_daily);
  int hq_tests_today = (int)(Tpars[1][3]*tests_available_daily);
...
//                  is_confined[ind[j]] = false; // Remove confinement (No need since hospitalised aren't confined)
                ...
                for(int j=0; j<targetted_tests_done_today;j++){
                  ...
                  if(day >= next_test_date[si] && being_tested[si]==false && tests_remaining_today>0){
                    ...
                    if(test_type==0){
                      ...
                      result_declared_date[si]= (int)(day + lq_delay);    // Days to wait before result result_declared_date
                      ...
                      result_declared_date[si]= (int)(day + hq_delay);    // Days to wait before result result_declared_date
              ...
              if(remaining_tests_done_today>0){               // If there are any tests remaining
                  ...
                  for(int j=0; j<remaining_tests_done_today;j++){
                    ...
                    if(test_type==0){
                      ...
                      result_declared_date[ri]= (int)(day + lq_delay);    // Days to wait before result result_declared_date
                      ...
                      result_declared_date[ri]= (int)(day + hq_delay);    // Days to wait before result result_declared_date
