...
double randomtestrate=0.1,testsensitivity=0.99,testspecificity=1,finishtestrate=24.0; // rate of testing people, test sensitivity, rate at which result is declared
...
for (int i=0;i<NP;i++) {homeloc[i]=(int)(2500*ran1(&idum1));links[i][0]=homeloc[i];links[i][1]=2500+(int)(250*ran1(&idum1));links[i][2]=i;linkweight[i]=1;}
}
	...
	for (i=0;i<NP;i++)
		switch(infectionstate[i])
			...
			case 0:// Susc to Asymptomatic
					r=rate[0][2]*(numinstateinloc[2][currentloc[i]]+numinstateinloc[3][currentloc[i]]+numinstateinloc[5][currentloc[i]])*deltat;
	...
	for (i=0;i<NP;i++)
	  if (testingstate[i]==0)// && infectionstate[i]!=5)
		if (ran1(&idum1)<randomtestrate*deltat)
			{
			numtests++;
			if (((infectionstate[i]==2 || infectionstate[i]==3 ||infectionstate[i]==5) && ran1(&idum1)<testsensitivity) || ((infectionstate[i]<=1 || infectionstate[i]==4) && ran1(&idum1)>testspecificity)) testingstate[i]=1;
...
fprintf(fpt,"%lf ",t);for (j=0;j<Numstates;j++)if(j!=1)fprintf(fpt,"%d ",numinstate[j]);fprintf(fpt,"%d %d %d\n",nummove,numtests,numtestsfinished);
fclose(fpt);
}
